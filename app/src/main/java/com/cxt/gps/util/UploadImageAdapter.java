...
public class UploadImageAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {//创建ImageView
			...
			iv_image.setLayoutParams(new AbsListView.LayoutParams((ImageUtils.getWidth(context)) / 4  , (ImageUtils.getWidth(context)) /4));
		...
		if(getItem(position) == null ){//图片地址为空时设置默认图片
			...
			//获取图片缩略图，避免OOM
			Bitmap bitmap = ImageUtils.getImageThumbnail((String)getItem(position), (ImageUtils.getWidth(context))/ 4 , (ImageUtils.getWidth(context))/4);
